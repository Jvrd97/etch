...
- **AFK-тикет ведёт петля `issue-loop`,** а не ручная цепочка промптов: `Workflow({ name: 'issue-loop', args: { issue: 'issues/PHASE-03/backlog/NN-slug.md' } })`. Она проходит preflight → план → TDD → упрощение → три ревью (стандарты, вписанность, antivibe-гейт) → вердикт → отчёт в `.claude/loop-reports/`, до трёх раундов правок. Набор тикетов сразу — `issue-swarm`, порядок он считает по строке `**Blocked by**`. Петля не коммитит, не двигает тикет по lifecycle и не запускает graphify — это остаётся человеку. Устройство, args и почему тесты гоняются в обход docker — `.claude/workflows/README.md`.
