...
- **Deep, not shallow** (AGENTS.md §2): a simple interface hiding real logic. A
...
6. **Checks** — `ruff check`, `ruff format --check`, `mypy --strict app`, `alembic heads`
   ...
   (`.Codex/workflows/README.md` has the exact form).
7. **Simplify** — run the `simplifier` agent (`.Codex/agents/simplifier.md`) over the files
   you touched. Behaviour and tests stay the same.
8. **Review marker** — `# [review:need-review] <ticket-id>` + `# summary:` in every touched
   code file (AGENTS.md §9).
...
10. **After the code is green, run the simplify pass** (agent `simplifier`,
    `.Codex/agents/simplifier.md`) on the files you touched. It removes what
