...
таблицах).
## 2026-08-30 — PHASE-03/152 «Экран правил дня: новая версия канона без SQL»

Тронуто 5 файлов бэкенда (3 new, 2 mod). Новых таблиц и миграций нет: тикет живёт на
`day_rule_set` из `#86`, голова цепочки осталась `e6b8d0f2a4c7`.

- `app/crud/day_rules.py` — **new**: `publish_rule_set` одной транзакцией (`SAVEPOINT`) —
  закрыть версию, покрывающую новую дату, вставить новую. Отказ откатывает и закрытие, тем
  самым дырки между версиями не остаётся. `earliest_valid_from(today)` — завтра, ровно то
  число, которое цитирует и экран. Правки строки нет ни одной функцией: единственный путь
  изменить канон — новая версия. `IntegrityError` с именем `excl_day_rule_set_no_overlap`
  переводится в предложение, любая другая — пробрасывается как есть.
- `app/api/day_rules.py` — **new**: `GET /day-rule-sets` (история + `today` и
  `earliest_valid_from`, посчитанные сервером по границе суток), `GET /day-rule-sets/current`,
  `POST /day-rule-sets`. `PUT`/`PATCH`/`DELETE` не существует — это контракт, а не пропуск.
  422 — дата не в будущем, 409 — перекрытие.
- `app/schemas/day.py` — **mod**: `DayRuleSetPublish` (`extra="forbid"`, без `id` и `valid_to`;
  зона проверяется через `ZoneInfo`, потолок-исключение не ниже обычного, дни недели — ISO без
  повторов) и `DayRuleSetHistoryResponse`.
- `app/main.py` — **mod**: роутер в периметр API-key.
- `tests/test_day_rules_editor.py` — **new**, 32 теста: вердикты трёх прошедших дней
  посимвольно те же после публикации потолка в 7 ч; новая версия действует со своей даты, днём
  раньше — старая; перекрытие отвергается базой и возвращается словами без `ExclusionViolation`;
  неудачная публикация возвращает `valid_to` предыдущей версии на место (проверено снятием
  `begin_nested` — тест краснеет); дата сегодня/вчера/месяц назад — 422 с числом «самая ранняя»;
  ни одного метода правки под префиксом; история и `current`; 12 форм кривого черновика, ни одна
  не доезжает до таблицы.

Feedback loops (backend): pytest **670/670 green** (было 638), `ruff check` clean,
`ruff format --check` clean (142 файла), `mypy --strict app` clean (103 файла), `alembic heads`
— одна голова `e6b8d0f2a4c7`. Docker-демон не поднят, `make check` целиком **не отрабатывал**:
тесты шли против постгреса на localhost:5432, база `habit_tracker_test_fast3` (общая
`habit_tracker_test` в этот момент содержала таблицы соседних веток, и `drop_all` на ней падал).

## 2026-08-31 — PHASE-03/120: изоляция CLI на все четыре юзкейса + смоук

Тронуто 11 файлов (2 new, 9 mod). Флаги изоляции стояли только на чате: инсайты, онбординг
категорий и разбор дня запускали `claude -p` без них и тащили в запрос личную конфигурацию
хоста. Общая сборка запуска переехала в один объект, `generate` пошёл через него, а живой
смоук получил право валить пред-выкатный прогон, когда первый ход снова стоит десятки тысяч
токенов.

- `app/llm/cli.py` — **mod**: `IsolatedCli` — единственная в репозитории сборка аргументов,
  окружения и рабочего каталога `claude -p`; `ISOLATION_FLAGS` (пары «флаг — значение») и её
  плоская форма `ISOLATION_ARGS`; `CliInsightsClient.generate` запускается через неё и получает
  свой `--system-prompt` вместо агентной преамбулы CLI.
- **Найдено живым замером**: набор из `#111` был неполон. `--setting-sources ""` не отключает
  MCP-серверы — они приезжают из `<config dir>/.claude.json`, и `claude --help` отсылает за ними
  к `--strict-mcp-config`. Замер 2026-08-31 на CLI 2.1.250: 21 620 токенов первого хода без
  этого флага против 290 с ним. Флаг добавлен в набор.
- `app/llm/prompts.py` — **mod**: `ONESHOT_SYSTEM_PROMPT` — нейтральный промпт одноходовых
  вызовов. Домен остаётся у вызывающего: три юзкейса по-прежнему складывают свою инструкцию с
  данными в текст сообщения, меняется только то, чью преамбулу видит модель.
- `app/llm/chat/client.py` — **mod**: свои копии флагов и запуска процесса убраны, `CliChatClient`
  делегирует `IsolatedCli`; внешние методы (`build_argv`, `build_env`, `cwd`) и их тесты те же.
- `app/core/config.py` — **mod**: комментарий — `CHAT_CLAUDE_CONFIG_DIR`/`CHAT_CLI_CWD` теперь
  обслуживают все четыре юзкейса, а не один чат.
- `tests/test_cli_isolation_smoke.py` — **new**: живой вызов `claude`, порог
  `input_tokens + cache_creation_input_tokens < 1000`, проверка ответа на следы личных скиллов
  и хуков. Пропускается с внятной причиной, когда нет признака окружения `LLM_CLI_SMOKE=1`,
  бинарника или подписки; любой другой ненулевой код выхода — падение, так ловится обратная
  ошибка (чужой флаг, отломавший запуск).
- `tests/test_llm_cli.py`, `tests/test_chat_cli_args.py` — **mod**: набор флагов проверяется
  параметризованно и у обоих клиентов — тест краснеет при пропаже ЛЮБОГО флага у ЛЮБОГО из них;
  состав набора закреплён вместе со своим замером.
- `tests/conftest.py` — **mod**: автофикстура уводит каталог конфигурации и cwd CLI во временный
  каталог — ни один тест не пишет в `/data`.
- `tests/test_insights.py` — **mod**: сидирование дня перешло с `date.today()` на `today_local()`.
  Тест падал между полуночью и `DAY_START_HOUR`: запись оказывалась за окном периода, которое
  строится по границе суток. Второе определение «какое сегодня число» из тестов убрано.
- `Makefile` (бэкенд и `habit-tracker/`) — **mod**: `make smoke` (живой смоук) и
  `make preflight` = `check` + `smoke` — прогон перед выкатом. В обычный `pytest` смоук не входит.
- `docs/llm-cli-isolation.md` — **new**: что делают флаги, почему версия CLI влияет на их
  поведение, порог и правило его пересмотра, причины пропуска смоука.

Feedback loops (backend): pytest **680 passed, 2 skipped** (пропущены оба живых теста смоука),
`ruff check` clean, `ruff format --check` clean (143 файла), `mypy --strict app` clean
(103 файла), `alembic heads` — одна голова `e6b8d0f2a4c7`, миграций тикет не заводит. Фронт:
`bun test` 750/750, `bunx tsc --noEmit` чисто (кода фронта тикет не трогает). Docker-демон не
поднят, `make check` целиком **не отрабатывал**: тесты шли против постгреса на localhost:5432,
база `habit_tracker_test_fast3`. Живой смоук на этой машине до вызова не доехал: явный
`CLAUDE_CONFIG_DIR` отключает вход через связку ключей macOS, а `CLAUDE_CODE_OAUTH_TOKEN` тут
нет — замер 290 против 21 620 сделан руками той же командной строкой.
...
## 2026-09-03 — PHASE-03/97: контур входящих, личный ClickUp читается вручную

Вход в систему появился впервые. Задачи дня рождаются снаружи, а внутри для них не было
ни таблицы, ни HTTP-клиента: `httpx` стоял только в dev-зависимостях. Первый срез
доказывает цепочку целиком — от внешнего API до экрана.

- `app/models/inbox.py` — new, пять таблиц ADR-0016. Приватность выражена отсутствием
  колонок: тела письма, текста сообщения и вложений здесь негде хранить физически, и это
  проверяется тестом по списку колонок, а не обещается в комментарии. Дедупликация —
  естественный ключ `(source_id, external_id)`, тем же приёмом, что health-контур.
- `alembic/versions/*_inbound_signal_tables.py` — new, ревизия `c5e7a9b1d3f6` от
  `b4d6f8a0c2e5`. Пять таблиц в порядке FK, индексы на FK явные, сид четырёх выключенных
  источников. Обратимость проверена на чистой базе: 5 таблиц → `downgrade` → 0.
- `app/crud/inbox.py` — new, адаптер личного ClickUp и запись. Курсор двигается только
  после успешной записи: сдвинутый раньше означал бы пропущенные задачи, если запись
  упала. Вставка отличается от обновления через `xmax = 0` — постгресовый признак новой
  версии строки; сравнение `created_at = updated_at` тут не работает, `onupdate` до
  upsert-а не доходит, и обновление считалось бы вставкой (поймано тестом, а не ревью).
- `app/api/inbox.py`, `app/schemas/inbox.py` — new. Лента, справочник и ручной прогон.
  Отказ приезжает машинным кодом: «выключен», «нет адаптера», «нет токена» — три разных
  состояния. Тела ответа провайдера в диагностике нет никогда — только код состояния.
- `app/core/config.py` — mod, `CLICKUP_PERSONAL_TOKEN`. В базе лежит имя переменной, не
  значение; пустое значение — это «источник не подключён», а не поломка.
- `pyproject.toml` — mod, `httpx` переехал из dev в основные зависимости.
- `tests/test_inbox.py` — new, 10 тестов на замоканном транспорте, без сети.

Проверено обратным ходом: снятие проверки `is_active` даёт запрос в сеть у выключенного
источника (тест краснеет на пустом списке вызовов); замена `xmax = 0` на сравнение дат
краснит тест повторного прогона; своя арифметика поясов вместо `local_date()` краснит
тест про 00:30.

Не проверено вживую: настоящий воркспейс. Токен кладёт человек в `CLICKUP_PERSONAL_TOKEN`;
без него источник отвечает `no_credentials` и в сеть не идёт — это тоже проверено тестом.

Feedback loops (backend): pytest **1530 passed, 2 skipped**, `mypy --strict app` (190
файлов), `ruff check`, `ruff format`, `alembic heads` — одна голова `c5e7a9b1d3f6`.

## 2026-09-03 — PHASE-03/97 (продолжение): чат видит задачи из входящих

Половина, ради которой контур и строился. Чат запускает CLI с `--tools ""` — ни MCP, ни
файлов, ни сети, — поэтому «модель сходит в ClickUp и посмотрит» невозможно ни в каком
виде. Единственный путь данных внутрь разговора — именованная выборка, которая пишет о
себе строку в `chat_retrievals`, и седьмое имя реестра ровно этим и стало.

- `app/llm/chat/retrieval.py` — mod. `inbox_tasks`: заголовок, внешний id и ссылка
  обратно, по умолчанию неразобранное и тридцать строк. Тела в ответе нет и взяться ему
  неоткуда — контур его не хранит. Описание для промпта заведено в `QUERY_HINTS`; тест на
  совпадение ключей реестра и описаний ловит расхождение, из-за которого модель звала бы
  выборку несуществующим параметром.
- `tests/test_chat_retrieval.py` — mod. Опись имён стала семёркой (восьмое имя не
  появится без правки реестра, и правка видна в диффе), плюс два теста: задача доезжает
  до модели вместе со ссылкой; пустой контур отвечает нулём строк, а не отказом —
  «задач нет» и «источник не подключён» модель обязана различать.

Feedback loops (backend): pytest **1532 passed, 2 skipped**, `mypy --strict app`,
`ruff check`.

## 2026-09-03 — PHASE-03/98: ключи вводятся в интерфейсе, рабочий ClickUp подключается

Первый срез назвал секрет именем env-переменной, и подключение второго воркспейса стоило
захода на VPS с пересборкой контейнера. Решение ADR-0016 D8 пересмотрено (запись в самом
ADR): секрет живёт в строке источника зашифрованным.

- `app/inbox/credentials.py` — new, Fernet поверх ключа, выведенного HKDF из
  `SESSION_SECRET`. Второй секрет не заводится: это второе место, где он бывает пуст в
  проде, и второй способ потерять данные при ротации. Цена названа в докстроке — шифротекст
  в дампе есть, ключ в дампе отсутствует; против владельца машины это не защищает.
- `alembic/versions/*_source_credentials_in_row.py` — new, ревизия `d6f8a0c2e4b7`:
  `secret_ciphertext`, `settings` (jsonb), `label`, `poll_interval_s` плюс подписи
  заготовкам. Обратимо.
- `app/crud/inbox.py` — mod. `set_credentials`, `may_write_back`, `_secret_of` (сначала
  строка, потом окружение — иначе смена ключа в интерфейсе молча ничего не меняла бы).
  `ADAPTERS` ключуется **провайдером**: личный и рабочий ClickUp — один API, разные ключ и
  id воркспейса, и второй воркспейс обязан быть данными, а не веткой в коде.
- `app/api/inbox.py`, `app/schemas/inbox.py` — mod. `PUT /sources/{id}/credentials`,
  `PATCH /sources/{id}`. Наружу уходит `has_secret`, а не значение, длина или хвост.
- `tests/test_inbox.py` — mod, +7 тестов: сохранённый ключ не читается в строке и не
  уходит в ответе; введённое важнее окружения; окружение продолжает работать, когда ничего
  не введено; Alvion читается тем же адаптером и остаётся read-only.

Проверено обратным ходом: возврат `ADAPTERS` к паре `(provider, account)` краснит тест
рабочего воркспейса; чтение окружения раньше строки краснит тест приоритета.

Feedback loops (backend): pytest **1539 passed, 2 skipped** — плюс один падающий
`test_challenges.py::test_moving_the_boundary_moves_the_challenge_with_it`, который падает
и на чистом `main`: он зависит от часов машины (два пояса по разные стороны линии перемены
дат). Заведён отдельной задачей, к этой работе отношения не имеет. `mypy --strict app`
(192 файла), `ruff check`, одна голова `d6f8a0c2e4b7`.

## 2026-08-31 — PHASE-03/187: чат переписывает готовый план дня

Разговор собирал план только пустому дню; занятый отвечал 409 и «такой операции нет».
Открыта одна операция класса W2 — перезапись плана дня целиком. Решение и его цена —
`docs/PHASE-03/ADRs/in-review/ADR-0021-chat-rewrites-the-day-plan.md`.

- `app/schemas/chat.py` — mod. У `ChatDayPlanOp.op` второе значение `rewrite_day_plan` и
  метод `named_by(day_taken=)`. Правки по одной строке в схеме по-прежнему нет: полей,
  которыми можно указать на строку существующего плана, не заведено, и это граница, а не
  договорённость.
- `app/api/chat.py` — mod. Из `_day_plan_refusal` ушла причина «день занят», добавлен
  `_named_day_plan` (имя операции ставит сервер по базе, не модель), `_write_day_plan`
  пишет поверх существующего плана. Повторное применение уже применённой плашки — 409:
  второй проход резал бы ревизию заново.
- `app/schemas/day_plan.py` — mod. `to_document` принимает `known_ids` и проставляет
  строке её прежний id по коду. Это и есть перенос отметок: `replace_plan` поднимает
  отметку строки, чей id вернулся в документе.
- `app/crud/plan.py` — mod, `item_ids_by_code`. Читает пары колонок, а не сущности, по той
  же причине, что и `_stored_item_ids`: словарь берут перед удалением строк.
- `app/llm/chat/prompt.py` — mod. Раздел плана дня переписан: день целиком и никогда его
  кусок, коды остающихся строк сохранять. `CHAT_CONTEXT_VERSION` 4 → 5.
- `app/llm/chat/context.py` — mod. Карточка дня печатает код, жёсткость и критерий
  готовности каждой строки. Без кодов модель может только стереть прожитый день.
- `tests/test_chat_day_plan.py` — mod. Два имени операции, перезапись занятого дня,
  выживание отметки по коду, отказ на втором нажатии.
- `tests/test_chat_plan_schema.py` — mod. Список разрешённых операций расширен явной
  строкой; `rewrite_day_plan` назван единственным представителем W2. Слова `uncheck`,
  `delete`, `rename`, `replace` в именах схемы по-прежнему запрещены.

Проверено обратным ходом: возврат отказа `day_already_has_a_plan` краснит тест перезаписи;
`to_document` без `known_ids` краснит тест выживания отметки.

Feedback loops (backend): pytest **1544 passed, 2 skipped**, `mypy --strict app`
(192 файла) чисто, `ruff check` и `ruff format --check` чисто.

## 2026-09-01 — PHASE-03/189: служебные блоки не едут человеку, план доезжает до плашки

Два симптома с одного экрана. Плашки не было никогда, если модель ходила за данными;
сырой JSON печатался в пузыре простынёй. Причины разные, чинятся врозь.

- `app/api/chat.py` — mod. `parts` заменён на `answered`: заход, кончившийся блоком
  `need`, забывается **до** запуска выборок. Упади они — человеку ещё ничего не сказано,
  и пузырь с блоком `need` был бы худшим отчётом об этом. `TURN_PASS_SEPARATOR` исчез
  вместе с накоплением: заходов в сохранённом тексте теперь ровно один.
- `app/llm/chat/plan.py` — mod, `_json_objects`. Скобки считаются, а не берутся крайние,
  и план снимается с **последнего** объекта, несущего ключ `plan`. Прежний `extract_json`
  на ответе с двумя объектами возвращал span с прозой внутри — битый JSON и молчаливое
  отсутствие плашки. Скобка внутри строкового литерала не считается: `"дочитал главу
  про {} в JSON"` — законная строка плана.
- `tests/test_chat_retrieval.py` — mod, класс `TestWhatTheHumanSees`: блок `need` не
  доезжает до сохранённого ответа; план, приложенный после похода за данными, доезжает
  до плашки. Второй тест и есть воспроизведение бага.
- `tests/test_chat_plan.py` — mod, +2 юнита: последний объект с планом побеждает; объект
  без ключа `plan` не заслоняет тот, в котором план есть.

Проверено обратным ходом: возврат `extract_json` в `_plan_object` краснит оба новых
юнита и тест хода с выборкой.

Feedback loops (backend): `make check` зелёный — pytest **1549 passed, 2 skipped**,
...
ушло `WHERE state = 'all'` — ноль строк, и модель сказала человеку, что во входящих
ничего нет. Ноль строк и «такого состояния нет» — разные ответы.

- `app/llm/chat/retrieval.py` — mod. `InboxTasksParams.state` проверяется валидатором
  против `SIGNAL_STATES` из модели входящих. Словарь берётся оттуда, а не переписывается
  рядом списком: второй список разошёлся бы с первым молча, а молчание здесь и есть
  чинимая болезнь. Подсказка выборки называет все четыре значения и говорит, что `null` —
  единственный способ попросить все состояния сразу.
- `tests/test_chat_retrieval.py` — mod, класс `TestInboxState`, +4 теста: отказ
  `bad_params` на состоянии вне словаря, приём каждого из четырёх, приём `null`, и тест
  на то, что подсказка называет словарь целиком. Последний падает, если в модель добавят
  пятое состояние и забудут про подсказку.

Проверено обратным ходом: снятие валидатора краснит первый тест — `"all"` снова проходит
до базы и возвращает пустую выборку без единого признака ошибки.

Пустая лента входящих этим тикетом не чинится и им не вызвана: на 01.09.2026 все четыре
источника в проде выключены, `last_polled_at` пуст, `inbound_signals` пуста. Расписания
опроса нет вовсе — `poll_source` зовётся только ручкой.

## 2026-09-01 — PHASE-03/191: проба источника, которая ничего не расходует

Проверить ключ интеграции было нечем: «перечитать» отвечает числами и двигает курсор,
поэтому второй прогон честно возвращает ноль нового — и этот ноль неотличим от
неработающего ключа.

- `app/crud/inbox.py` — mod. `probe_source(source, transport=)` **не берёт сессию базы**,
  и это граница, а не экономия: функции без сессии нечем записать даже по недосмотру.
  Курсор снимается через новый `full=True` у адаптера — у читанного источника проба
  иначе показала бы пустой список на рабочем ключе.
- `app/crud/inbox.py` — mod. Отбор адаптера вынесен в `_adapter_for()`: один и тот же у
  опроса и у пробы, вместе со свойством «выключен = наружу не ушло ни одного запроса».
  Реестр `ADAPTERS` типизирован протоколом `Adapter` — второй адаптер с другой сигнатурой
  краснеет в реестре, а не в месте вызова.
- `app/schemas/inbox.py`, `app/api/inbox.py` — mod. `POST /sources/{id}/probe` →
  `{count, items}`. `count` считает всё, `items` обрезаны потолком 100: «показано 100» на
  воркспейсе из двух тысяч задач не должно читаться как «их сто». Полей у элемента
  четыре — те же, что кладёт в базу опрос; тела нет и здесь (ADR-0016, D2).
- `tests/test_inbox.py` — mod, класс `TestProbe`, +5 тестов: показывает задачи; не пишет
  ни сигналов, ни курсора, ни `last_polled_at`; игнорирует курсор; отвергает выключенный
  источник без единого запроса наружу; ручка отдаёт список.

Проверено обратным ходом: снятие `full=True` краснит тест про курсор — проба на читанном
источнике начинает возвращать пустой список.

## 2026-09-01 — PHASE-03/193: потолок реплики поднят под вложения

- `app/schemas/chat.py` — mod. `MESSAGE_MAX_CHARS` 20 000 → 100 000. В реплику стало можно
  приложить текстовый файл, а хранилища вложений в проекте нет: содержимое файла и есть
  часть сообщения. Прежний потолок не вмещал конспект дня. Сто тысяч знаков ≈ двадцать
  пять тысяч токенов на ход — дорого, но договоримо, и платит за это человек осознанно,
  приложив файл руками. Константа используется в одном месте — `MessageCreate.content`.

## 2026-09-01 — PHASE-03/195, 196: свежие входящие и канон числами

Два разных «оно не работает» с одной причиной в основе — сервер знал то, чего не знали
ни модель, ни человек.

- `app/crud/inbox.py` — mod, `refresh_stale()` и `_is_stale()`. Активные источники с
  вышедшим сроком опрашиваются перед чтением; `poll_interval_s` до этого лежал в таблице
  мёртвым — читать его было некому. Отказ провайдера возвращается кодом, а не исключением:
  освежение — удобство поверх чтения, и упавший ClickUp не должен ронять разговор о дне.
- `app/llm/chat/retrieval.py` — mod. `inbox_tasks` освежает источники перед выборкой.
- `app/llm/chat/context.py` — mod, секция «Правила дня». Промпт называет правила словами,
  `check_all` судит по числам из `day_rule_set`, и этих чисел модель не видела нигде.
  Наблюдалось в проде: якорь отношений на 19:00-20:00 при свободном вечере 19:10-21:00 —
  оба правила промпта соблюдены, канон нарушен, плашки нет. Щель между пересекающимися
  вечерами считается, а не пишется словами: канон менялся дважды за месяц.
- `app/llm/chat/prompt.py` — mod. `CHAT_CONTEXT_VERSION` 5 → 6; раздел правил говорит
  брать числа из секции, а не на глаз.
- `tests/test_inbox.py`, `tests/test_chat_day_card.py` — mod, +8 тестов.
...
Изменено 5 файлов: `app/models/field.py` (mod), `app/schemas/category.py` (mod), `app/crud/category.py` (mod), `tests/test_categories.py` (mod), `alembic/versions/2026_09_05_1300-a9c1e3f5b7d0_field_units_quick_steps.py` (new).
