  ...
  entriesAPI: {
    getAll: (params?: EntriesQuery) => {
      ...
      if (params?.categoryId !== undefined) {
        ...
        return Promise.resolve(categoryHistory.slice(skip, skip + (params.limit ?? 100)));
      }
...
describe('useDashboard hero', () => {
  ...
  it('paginates bounded category history beyond one page for Continue tracking', async () => {
    categoryHistory = Array.from({ length: 101 }, (_, i) => entry(i + 1, TODAY, String(i)));
    const { result } = renderHook(() => useDashboard());
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.continueEntries).toHaveLength(101);
    const categoryCalls = entriesCalls.filter((call) => call.categoryId === WATER.id);
    expect(categoryCalls).toHaveLength(2);
    expect(categoryCalls[0].limit).toBeGreaterThan(0);
    expect(categoryCalls[1].skip).toBe(categoryCalls[0].limit);
  });
});
