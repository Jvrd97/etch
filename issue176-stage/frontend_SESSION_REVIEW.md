...
## 2026-08-31 — PHASE-03/120 живой чат: ход мысли, признак жизни, копирование

Фронтовая половина хода `#120`. Бэкенд провёл наружу пять шагов хода (`thinking`,
`writing`, `acting`, `step_end`, `stop`); экран до этой правки читал из потока одно
событие — `delta`, — и пауза до первого слова выглядела одинаково при думающей модели и
при мёртвом процессе.

Граница приватности продолжена с бэкенда, а не заведена заново: слова мысли живут в
`TurnProgress.thinking`, ответ — в `turn.text`, и пузырь модели собирается только из
второго. Мысль никуда не сохраняется — ни в базу (её туда и не шлют), ни в
`localStorage`: черновик пишется отдельным механизмом и мысли не видит.

Ширина строки в ленте переехала с процента контейнера на меру в буквах: прежние
`max-w-[90%]` внутри `max-w-7xl` давали строку под тысячу пикселей. Акцент оставлен там,
где он и был, — у своей реплики и у кнопок; ответ модели цвета не менял (он и раньше был
`bg-card`).

- `lib/chat-stream.ts` — **mod**: пять новых видов события в разборе; мысль приезжает в
  поле `thinking`, никогда не в `text`.
- `lib/chat-progress.ts` — **new** (+тест): состояние «чем модель занята», склейка слов
  мысли, оценка объёма, подпись свёрнутой строки; на событии, которое ничего не меняет,
  возвращается тот же объект.
- `components/chat/ThinkingBlock.tsx` — **new**: свёрнутый блок мысли; сворачивается сам
  на переходе «ответ пошёл» правкой состояния в рендере, а не эффектом.
- `components/chat/TurnLive.tsx` — **new**: три точки до первого слова и курсор после;
  `prefers-reduced-motion` разбирается в CSS, а не чтением `matchMedia`.
- `components/chat/CopyButton.tsx` — **new** (+тест): копирование сообщения; отказ буфера
  оставляет кнопку молчащей, а не врущей.
- `components/chat/ChatBubble.tsx` — **new**: один пузырь на обе оболочки — мера строки,
  перенос сплошного токена, место под кнопку копирования.
- `components/chat/ChatFeed.tsx` — **mod**: локальный `Bubble` заменён общим, мысль стоит
  над ответом, признак жизни и курсор внутри пузыря модели.
- `components/chat/ChatFeed.live.test.tsx` — **new**: 14 тестов живой половины ленты.
- `hooks/useChat.ts` — **mod**: шаги складываются в `turn.progress`; разбор события по
  видам вместо `else`, который при новых видах означал бы «мысль дописана в ответ».
- `app/chat/page.tsx` — **mod**: та же тройка на широком экране, свой `Bubble` удалён.
- `app/m/chat/page.test.tsx` — **mod**: три теста идущего хода на обеих оболочках сразу.
- `app/globals.css` — **mod**: `.chat-dot`, `.chat-caret`, `.copy-affordance` и правило
  сокращённого движения для всех трёх.
- `lib/chat-stream.test.ts`, `lib/chat-progress.test.ts` — тесты разбора и состояния.

Проверки (frontend): `bun test` **1271 pass, 0 fail** (114 файлов, было 1234),
`bunx tsc --noEmit` clean, `bun run lint` 0 errors (13 warnings — прежние чужие
`_`-параметры, новых не добавилось), `bun run build` собирается, классы меры и анимаций
доехали до CSS. Бэкенд не тронут и проверен на несломанность: **1504 passed, 2 skipped**,
`ruff`/`mypy --strict` чисто.

Проверка «падает, если мысли перестанут показываться»: убран разбор `thinking` в
`applyProgress` → **4 failed**; убран `<ThinkingBlock>` из ленты → **6 failed**; убран
признак ожидания → **3 failed**; убрана кнопка копирования → **2 failed**; возвращена
прежняя ширина строки → **2 failed**. После восстановления — 1271 pass.
## 2026-08-31 — PHASE-03/148 кнопка «собрать план» на дне без плана

Карточка «Плана нет» перестала быть тупиком: обе ручки сборки лежали на сервере, но экран их не звал, а `dayAPI.generatePlan` в клиенте не существовало вовсе.

- `lib/api.ts` — **mod**: `dayAPI.generatePlan` — `POST /day/{date}/plan/generate`. 503 у ручки нет: ответ всегда несёт план, `source` говорит, кем он собран.
- `hooks/usePlanBuild.ts` — **new**: состояние сборки размеченным union (`idle | generating | skeleton | failed`), а не парой флагов; второй клик игнорируется рефом, потому что два `POST` подряд затирают друг друга через `replace_plan`.
- `components/day/PlanBuilder.tsx` — **new**: карточка «плана нет» с выходом из неё. Генерация — основной путь, скелет предлагается только после отказа; ожидание сказано словами (`role="status"`), отказ — сообщением, а не мёртвой кнопкой. Один компонент на оба шелла.
- `components/DayScreen.tsx`, `components/mobile/MobileDayScreen.tsx` — **mod**: ветка `plan === null` отдана `PlanBuilder`; подпись «почему скелет» перекрашена в `text-warning` — урезанный план виден, а не спрятан сноской.
- `test-fixtures/day.ts` — **new**: фикстура дня и плана, вынесенная из `DayScreen.test.tsx`, чтобы оба шелла проверялись против одного дня.
- `components/day/PlanBuilder.test.tsx` — **new**: кнопка зовёт генерацию с датой этого дня, ожидание видно, второй клик не пишет второй план, отказ показан текстом и только тогда предлагает скелет, успех возвращает день на перечитывание.
- `components/DayScreen.test.tsx` — **mod**: кнопка есть только когда плана нет, после сборки план отрисован, скелет назван скелетом с причиной, нарушения собранного плана видны.
- `components/mobile/MobileDayScreen.test.tsx` — **new**: тот же выход на телефоне.

Проверка обратная: при `onClick={() => {}}` падают 6 тестов `PlanBuilder`, при вырезанном `<PlanBuilder>` из шеллов — 5 тестов экранов.

Проверки: `bunx tsc --noEmit`, `bun run lint` (0 errors), `bun test` — 1249 passed, `bun run build`.
...
## 2026-08-31 — PHASE-03/nav-drawer, десктопная навигация в выдвижной панели

Горизонтальная строка из семнадцати пунктов убрана. Она не чинилась сжатием: подписи
требуют ≈2000px внутри контейнера, упёртого в `max-w-7xl` (1216px доступно), поэтому
хвост срезался `overflow-x-hidden` на `<body>` и до `Day rules` не было клика вообще.

Снаружи остались два экрана, которые открывают каждый день, — `Today` и
`Быстрые отметки`. Остальные пятнадцать — в панели, тремя группами: **День**
(today, day, life, week, chat, daily-summary), **Данные** (dashboard, roles, table,
entries, journal, insights), **Настройка** (goals, quick-marks, categories, day-rules,
onboarding). Ось группировки — частота обращения, она уже была записана в комментариях
реестра у каждого `inTabBar: null`.

`ViewToggle` остался смонтированным в шапке, а не уехал в панель: он несёт
cold-start restore в `/m`, и восстановление, которое работает только при открытой
панели, не работает. `LogoutButton` тоже остался снаружи — это единственный выход
из сессии.

Активность считается по `isActiveRoute`: равенство плюс вложенность, поэтому
`/day/2026-08-30`, `/week/2026-W35`, `/categories/12` и `/roles/rules` наконец
подсвечивают свой экран. Раньше на них не был подсвечен ни один пункт.

Мобильная оболочка не тронута: `TAB_BAR_ROUTES`, `MORE_ROUTES` и `MOBILE_TABS`
фильтруют по `inTabBar`, новые поля им ортогональны; порядок записей в `APP_ROUTES`
не менялся.

Файлов тронуто: 11 (4 new, 7 mod).

- `components/NavDrawer.tsx` — new, панель: секции, активный маршрут, Escape, ловушка Tab, замок прокрутки, подложка.
- `lib/focus-trap.ts` — new, `FOCUSABLE_SELECTOR` + `trapTab`, общие с `FullScreenSheet`.
- `components/Navigation.test.tsx` — new, 16 тестов: якоря снаружи, открытие/закрытие кнопкой, Escape, подложка, возврат фокуса, ловушка Tab, активный маршрут на детальном роуте.
- `components/AppShell.test.tsx` — new, 4 теста: панель только у десктопной оболочки, `/m/*` и `/login` получают голые children.
- `components/Navigation.tsx` — mod, шапка переписана: кнопка-гамбургер, логотип, два якоря, `ViewToggle`, «Выйти».
- `lib/routes.ts` — mod, поля `section` и `inHeader`, экспорты `NAV_SECTIONS`/`HEADER_ROUTES`/`SECTION_NAMES`/`SECTION_ORDER` и `isActiveRoute`.
- `lib/routes.test.ts` — mod, +18 тестов на группировку, якоря и активность.
- `lib/ui-constants.ts` — mod, `navRowClass` и слова панели переехали сюда из `MoreSheet`.
- `components/mobile/MoreSheet.tsx` — mod, локальная `ROW_CLASS` заменена общей `navRowClass`.
- `components/mobile/FullScreenSheet.tsx` — mod, ловушка Tab вынесена в `lib/focus-trap`, поведение прежнее.
- `app/globals.css` — mod, `drawer-in`/`scrim-in` и их отключение под `prefers-reduced-motion`.

Мутационная проверка тестов (каждая правка откатывалась): кнопка перестала открывать
панель — 12 падений; Escape не обрабатывается — 2; ловушка Tab выключена — 2; клик по
подложке мёртв — 1; `insights` переложен в другую группу — 2.

Feedback loops (frontend): `bun test` **1266 pass, 0 fail** (113 файлов),
`bunx tsc --noEmit` clean, `bun run lint` 0 errors (13 warnings — чужие `_`-параметры
в тестах, не тронуты), `bun run build` clean.

## 2026-08-31 — PHASE-03/92: якорь отмечается без перезагрузки экрана

Жалоба с экрана дня: клик по якорю выглядит как перезагрузка всей страницы. Причина не в
роутере — в `useDay`. `reload()` после записи поднимал общий `loading`, а `DayScreen`
на `loading` возвращает `<LoadingSpinner/>` вместо всего экрана: день разбирался и
собирался заново. Сама галочка при этом ждала круга до сервера — блок не менялся, пока
день не прочитан повторно.

Две правки, каждая закрывает свою половину: отметка встаёт локально, перечитывание идёт
тихо. Тронуто 5 файлов.

- `hooks/useDayAnchors.ts` — new, состояние якорей: клик применяется до запроса, `done` и
  «Не закрыты» пересчитываются по формуле сервера (`CLOSING_ANCHOR_STATES` = `done` +
  `skipped`), ответ `PUT` замещает догадку, отказ откатывает и пробрасывается наружу.
- `hooks/useDayAnchors.test.ts` — new, 7 тестов: галочка до ответа, `skipped` считается
  закрытым, запись именует `kind`, ответ сервера побеждает, откат при отказе,
  перечитанный день побеждает, новый объект-пропс не роняет экран в цикл.
- `hooks/useDay.ts` — mod, `loading` теперь только про «показывать нечего»; повторное
  чтение того же дня поднимает отдельный `refreshing` и день с экрана не убирает.
  Ключ — `shownKey`: дата, которая сейчас нарисована, пустая после ошибки.
- `hooks/useDay.test.ts` — mod, +2 теста: reload не гасит уже показанный день; смена даты
  по-прежнему показывает спиннер.
- `components/DayScreen.tsx`, `components/mobile/MobileDayScreen.tsx` — mod, оба экрана
  берут якоря из хука; `reload()` после записи остался — вердикт дня стоит на якорях, —
  но теперь он тихий.

Проверка мутацией: убран локальный пересчёт `missing` — 2 падения; откат при ошибке
заменён на «оставить как есть» — 1; сравнение по `signature` заменено на сравнение
ссылок — 1 (тест на цикл).

Feedback loops (frontend): `bun test` **1275 pass, 0 fail** (114 файлов),
`npx tsc --noEmit` clean, `bun run lint` 0 errors (13 warnings — те же чужие
`_`-параметры в тестах).

Проверено в браузере (дев-база догнана до `b4d6f8a0c2e5`, фронт на 3010 против бэкенда
на 8010). Замер вокруг клика по якорю «старт работы»: через 30 мс галочка стоит, счётчик
«1 из 6», `[role=status][aria-label=Loading]` на странице нет, заголовок дня на месте;
через 2.5 с — то же самое, уже с ответом сервера. Кольцо пройдено до конца
(сделано → не сделал → пусто), состояние якоря возвращено в исходное.
||||||| f37560c

## 2026-08-31 — PHASE-03/111: история разговоров рядом с лентой

Чат был одноразговорным: экран брал свежий разговор или заводил новый, а вернуться к
вчерашнему было нельзя — список существовал только на сервере. Разговор теперь адрес
(`?conversation=<id>`), а слева от ленты стоит история. Тронуто 6 файлов, 4 из них новые.

- `lib/chat-history.ts` — new, чистая часть: заголовок строки (сервер пишет его по первой
  реплике; у брошенного разговора его нет), часы последней реплики, разбивка ленты по дню
  начала — «Сегодня», «Вчера», дальше датой. Порядок внутри дня серверный и здесь не
  трогается: второй порядок на экране означал бы два ответа на «какой разговор свежее».
- `lib/chat-history.test.ts` — new, 9 тестов, включая устойчивость ключа группы: день
  группы — это React-ключ, и `undefined` в нём означает список, тихо переставший различать
  свои строки.
- `hooks/useChatHistory.ts` — new, лента, создание разговора и перечитывание. Заведённый
  разговор встаёт в начало списка до перечитывания — экран уходит в него сразу, и строка,
  на которую человек смотрит, обязана уже быть. Защёлка на `useRef` от двойного клика:
  два клика попадают в один рендер и видят одно и то же состояние.
- `hooks/useChatHistory.test.ts` — new, 6 тестов: одно чтение ленты, оптимистичная строка,
  названная ошибка вместо пустой истории, откат при отказе, перечитывание, защёлка.
- `components/chat/ChatHistory.tsx` + тест — new, 8 тестов. Строки — ссылки, а не кнопки:
  разговор это адрес, и список, переключающий состояние, отнял бы у чата «назад» и
  возможность прислать ссылку. Открытая строка помечена `aria-current`, а не только цветом.
- `app/chat/page.tsx` — mod, экран читает `?conversation`, история стоит рядом всегда — в
  том числе пока лента грузится, — а `useSearchParams` потребовал границы `Suspense`.
  Лента перечитывается после каждого хода: заголовок разговору пишет первая реплика.

Проверено в браузере против живого бэкенда: список с группами «Сегодня»/«Вчера»,
переключение по клику, создание разговора с переходом в него, подсветка открытого.

Feedback loops (frontend): `bun test` **1294 pass, 0 fail**, `tsc --noEmit` чисто,
`bun run lint` 0 errors, `bun run build` чисто.

## 2026-09-03 — PHASE-03/97: экран «Входящие»

Фронтовая половина первого среза контура входящих. Экран отвечает на два вопроса: что
приехало снаружи и почему источник молчит.

- `app/inbox/page.tsx` — new, справочник источников с кнопкой «перечитать» у каждого и
  лента сигналов со ссылкой обратно. Три состояния источника — «не подключён», «адаптера
  нет» и машинный код последнего отказа — показаны раздельно, а не свёрнуты в одну серую
  строку: они требуют разных действий.
- `hooks/useInbox.ts` — new, лента и справочник одним чтением, ручной прогон по одному
  источнику за раз (защёлка на `useRef`: два клика попадают в один рендер и боролись бы
  за курсор источника), отказ переводится из машинного кода в предложение.
- `lib/api.ts` — mod, `inboxAPI` и три типа. `credential_ref` уходит на экран именно
  именем переменной: видно, чего не хватает, без показа секрета.
- `lib/routes.ts` — mod, запись `inbox` в группе «Данные», без мобильного близнеца.
- `lib/routes.test.ts`, `lib/view-mode.test.ts` — mod, описи маршрутов пополнены. Список
  экранов без мобильной формы — явный: следующий такой придётся дописать осознанно.
...
## 2026-08-31 — PHASE-03/187: план дня в плашке чата

Операция `day_plan` существовала на сервере с `#115`, но плашка её не рисовала вовсе:
предложенный план дня не имел ни строки, ни галочки, и до кнопки не доезжал ни на пустом
дне, ни на занятом. Здесь она появляется вместе с перезаписью.

- `components/ChatPlanCard.tsx` — mod. План дня — одна галочка на весь день, а не галочка
  на строку: он пишется целиком или никак. Перезапись названа перезаписью и перечисляет
  коды, по которым переносятся отметки; имя операции читается из предложения, экран его не
  вычисляет.
- `components/ChatPlanCard.test.tsx` — mod, +4 теста: строка плана дня считается одной
  операцией, перезапись говорит о замене, флаг уходит булевым, снятая галочка шлёт `false`.
- `lib/api.ts` — mod. `ChatPlanDayPlanOp` с двумя именами операции, `day_plan` в теле плана,
  в выборе человека и `day_plan_id` в ответе применения.

Feedback loops (frontend): `bun test` **1361 pass, 0 fail**, `tsc --noEmit` чисто,
`bun run lint` 0 errors.

## 2026-09-01 — PHASE-03/189: пузырь показывает ответ, а не переписку с сервером

Блоки `{"need": ...}` и `{"plan": ...}` печатались в ленте дословно. Оба адресованы
серверу: первый отражён строкой выборки под ответом, второй — плашкой с галочками.

- `lib/chat-answer.ts` + тест — new, 7 тестов. `visibleAnswer()` считает скобки, находит
  объекты со служебными ключами и вырезает их вместе с забором из тройных кавычек.
  Обычный блок кода и скобка внутри строки плана не трогаются. Режется только показ:
  в `chat_messages.content` лежит ровно то, что ответила модель.
- `hooks/useChat.ts` — mod. Кадр выборки сбрасывает накопленный живой текст: то, после
  чего пришла выборка, было блоком `need`. Без сброса человек видел бы сырой JSON до
  первой перезагрузки, а после неё — нет, и живая лента расходилась бы с сохранённой.
- `app/chat/page.tsx`, `components/chat/ChatFeed.tsx` — mod. Обе ленты показывают
  `visibleAnswer`, копируется тоже видимое: копия того, чего на экране нет, — отдельная
  неправда.

Не сделано намеренно: `/m/chat` плашку не рисует вовсе — состояние планов живёт в
десктопной странице, а не в общем хуке. Поднимать его — отдельная работа, названа в
тикете.

Feedback loops (frontend): `bun test` **1369 pass, 0 fail**, `tsc --noEmit` чисто,
`bun run lint` 0 errors.

## 2026-09-01 — PHASE-03/191: debug-блок под ключом источника

Под полем ключа на карточке источника — кнопка «проверить» и то, что источник отдаёт
прямо сейчас. Ничего не записывается, курсор не двигается, жать можно сколько угодно.

- `components/inbox/SourceCard.tsx` — mod. Блок появляется только после нажатия: пустая
  рамка на каждой карточке говорила бы о состоянии, которого нет — «пробовали и ничего»
  и «не пробовали» разные ответы. Три исхода разведены: список с ссылкой обратно, «ключ
  ответил, но задач не видно» и отказ словами на той карточке, которая отказала.
  Обрезанный потолком список называет остаток («видно задач: 214 — показаны первые 100»).
- `hooks/useInbox.ts` — mod. `probe(sourceId)` и `probes: Record<number, ProbeState>`.
  `ProbeState` — размеченное объединение, а не пара «список плюс ошибка»: пара позволила
  бы выразить четвёртое состояние, которого не бывает.
- `lib/api.ts` — mod, `inboxAPI.probe`, `ProbeItem`, `ProbeOutcome`.
- `components/inbox/SourceCard.test.tsx` — mod, +6 тестов на три исхода, на потолок, на
  запертую кнопку без ключа и на выключенном источнике.

Feedback loops (frontend): `bun test` **1376 pass, 0 fail**, `tsc --noEmit` чисто,
`bun run lint` 0 errors.

## 2026-09-01 — PHASE-03/191 (добавка): запертая кнопка называет причину

«Проверить не горит» — первый же вопрос после выката. Кнопка была тёмной и молчала,
хотя причина серверу известна и однозначна.

- `components/inbox/SourceCard.tsx` — mod. Под кнопками строка с тем, что стоит на пути:
  «источник выключен — включите его галочкой справа» или «ключа нет — задайте его полем
  выше и сохраните». Порядок причин тот же, каким отвечает сервер: сначала «выключен»,
  потом «нет ключа». Когда путь свободен, строки нет вовсе — подсказка о состоянии,
  которого нет, хуже молчания.
- `components/inbox/SourceCard.test.tsx` — mod, +2 теста: причина названа в обоих
  случаях; объяснение исчезает, когда мешать нечему.
...
## 2026-09-01 — PHASE-03/193: файл в реплику текстом, без хранилища

Выбран самый дешёвый из трёх вариантов: содержимое файла становится частью реплики и
нигде не хранится. Ни ручки загрузки, ни тома, ни таблицы вложений — значит и удалять
потом нечего.

- `lib/chat-attachment.ts` + тест — new, 8 тестов. `isTextFile()` судит по типу от
  браузера и по расширению: у `.md` и `.log` тип часто пуст. `attachToDraft()` считает
  потолок по всей реплике, а не по файлу — почти полное поле плюс маленький файл упираются
  в него так же, как пустое поле плюс огромный.
- `components/chat/ChatComposer.tsx` + тест — mod, +3 теста. Перетаскивание и кнопка
  «Приложить файл»; отказ показывается тут же и называет файл по имени. Картинка
  отвергается, а не вклеивается байтами: прочитанный как текст бинарник приезжает мусором
  и тратит потолок целиком.

## 2026-09-01 — PHASE-03/194: проба говорит, что ничего не записала

Проба показала 100 задач, человек прочитал это как «всё приехало», и следующим вопросом
стало «почему чат их не видит». Чат читает `inbound_signals`, а проба туда не пишет.

- `components/inbox/SourceCard.tsx` + тест — mod, +2 теста. После удачной пробы на
  непрочитанном источнике под числом стоит «это ещё не в трекере: проба только смотрит».
  Подсказка исчезает сама, как только `last_polled_at` появился.

Feedback loops (frontend): `bun test` **1396 pass, 0 fail**, `tsc --noEmit` чисто,
`bun run lint` 0 errors.

## 2026-09-01 — PHASE-03/196: отвергнутый план больше не молчит

Модель написала «вот полный план на сегодня», сервер его отверг каноном, блок из показа
вырезан — и под сообщением осталась пустота. Выглядит как поломка ленты.

- `lib/chat-answer.ts` + тест — mod, +3 теста. `carriesPlan()` отвечает на один вопрос:
  блок плана в ответе был? Блок `need` планом не считается — он про данные, а не про запись.
- `app/chat/page.tsx` — mod. Под ответом, который нёс план и не получил плашки, стоит
  «предложение не прошло проверку правил дня». Сервер для этого не менялся: признак
  считается из текста и пустого `plan_id`.

Причину экран не называет — она осталась машинным кодом в логе. Назвать её человеку это
колонка в `chat_messages` и миграция, отдельная работа.
...
Изменено 25 файлов: `lib/api.ts` (mod), `lib/format-value.ts` (new), `lib/format-value.test.ts` (new), `lib/entry-values.ts` (mod), `lib/entry-values.test.ts` (mod), `lib/routes.ts` (mod), `lib/routes.test.ts` (mod), `components/QuickNumberRow.tsx` (mod), `components/QuickNumberRow.test.tsx` (mod), `components/EntryForm.tsx` (mod), `components/mobile/EntryEditorSheet.tsx` (mod), `components/ContinueTracking.tsx` (new), `components/ContinueTracking.test.tsx` (new), `hooks/useCategories.ts` (mod), `hooks/useCategories.test.ts` (mod), `hooks/useDashboard.ts` (mod), `hooks/useDashboard.test.ts` (mod), `app/categories/page.tsx` (mod), `app/m/categories/page.tsx` (mod), `app/dashboard/page.tsx` (mod), `app/m/dashboard/page.tsx` (mod), `app/entries/page.tsx` (mod), `app/entries/page.test.tsx` (mod), `app/m/entries/page.tsx` (mod), `app/m/entries/page.test.tsx` (mod).
