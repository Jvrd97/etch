...
// summary: pure reading of the day's violations — which lines a rule was found on, the rule spelled in Russian, how much a violation costs, and who assembled the plan that is on screen

import type { Plan, PlanRuleCode, PlanViolation } from '@/lib/api';
 ...
 */
/**
 * Бейдж над планом, собранным ночью и никем не смотренным (`#151`).
 *
 * Ночной прогон строит только скелет, и человек утром должен видеть это, а не
 * думать, что план кто-то продумал. Пометка снимается первой правкой или первой
 * отметкой — действием с планом, а не кнопкой «я посмотрел».
 */
export const NEEDS_REVIEW_BADGE = 'Собран ночью, не проверен';
