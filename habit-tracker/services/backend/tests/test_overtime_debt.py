...
class TestLedger:
    ...
    async def test_the_debt_counts_from_the_baseline_under_a_raised_ceiling(
        ...
        Акт роли закрывает клауз `#137`: под слитым каноном рабочий день без
        него проигран по `role_act`, и тогда тест мерил бы не потолок.
        """
        await record_role_act(db_session, MONDAY)
        await db_session.commit()
        await client.post(
...
async def test_the_same_day_is_lost_once_the_raise_has_expired(
    ...
    Акты роли стоят на обоих днях, чтобы клауз `#137` не решал за потолок:
    вторник обязан проиграть именно по `overtime`.
    """
    await record_role_act(db_session, MONDAY)
    await record_role_act(db_session, TUESDAY, title="ADR-0021: потолок дня")
    await db_session.commit()
    await client.post(
        f"{RULES_URL}/activations",
        json={
