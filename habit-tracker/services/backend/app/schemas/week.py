...
class WeekResponse(BaseModel):
    ...
    debt_minutes: int = Field(
        0,
        description=(
            "Минуты переработки этой недели, которые ещё не вернулись. Пока их "
            "больше нуля, неделя не выиграна, как бы ни судился каждый её день"
        ),
    )
    is_won: bool = Field(
        False,
        description=(
            "Все дни недели выиграны, дни в ней есть и долга за переработку нет. "
            "Гибкость потолка покупается возвратом, а не выдаётся"
        ),
    )
