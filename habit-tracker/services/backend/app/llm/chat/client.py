...
from app.llm.cli import CLI_BINARY, CLI_MODEL_LABEL, terminate_process
...
class CliChatClient(ChatLLMClient):
    ...
    async def stream_turn(
        ...
        try:
            ...
            # разговор, который его породил. `terminate_process` доводит
            # `kill` + `wait` до конца и под отменой: без щита `await` здесь
            # получил бы `CancelledError` немедленно и оставил процесс жить.
            await terminate_process(process)
